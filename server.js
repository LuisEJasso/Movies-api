...
//configurar para poder recibir el body
...
app.post('/movie', (req,res) =>{
    ...
    Movie.create(movie, (err, newMovie) =>{
        err ? res.status(409).send(err) : res.status(201).send(newMovie);
    });
});
...
app.get('/movie/:id', (req,res) =>{
    ...
    .then((movie) =>{
        movie ? res.status(200).send(movie) : res.status(404).send({message: "Movie no encontrada"});
    })
...
app.post('/director', (req,res) =>{
    ...
    Director.create(director, (err, newDirector) =>{
        err ? res.status(409).send(err) : res.status(201).send(newDirector);
    });
});
...
app.get('/director/:id', (req,res) =>{
    ...
    .then((director) =>{
        director ? res.status(200).send(director) : res.status(404).send({message: "Director no encontrado"});
    })
    .catch((error) => res.status(409).send(error))
});
...
app.patch('/director/:id', (req, res) =>{
    ...
    .then((director) =>{
        director ? res.status(200).send(director) : res.status(404).send({message: "Director not found"})
    })
    .catch((error) => res.status(409).send(error))
});
